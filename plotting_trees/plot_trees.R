...
# load trees ####
phy_strict <- read.nexus("parsimony/myStrictConsensustree.tre")
...
phy_adams <- read.nexus("parsimony/myAdamsConsensustree.tre")
...
phy_boot <- read.nexus("parsimony/NUFV137_phylogenetic_analysis_main_ms_bootstrap500_con.tre")
...
phy_bayesian <- read.nexus("bayesian/NUFV137_tree_all_mrb.con.tre")
phy_bayesian <- ladderize(phy_bayesian, right = TRUE) 
phy_bayesian <- root(phy_bayesian, outgroup = "Megalichthys", resolve.root = TRUE)

phy_decay <- read.nexus("autodecay/myStrictConsensustree.tre")
...
pdf("plotting_trees/plot_all.pdf", # File name 
    width = 15, height = 12)
...
pdf("plotting_trees/plot_Fig4.pdf", # File name 
     width = 12, height = 8)
